...
## Cuotas del plan gratuito y cómo esta app las cuida

Firebase Firestore, en su plan gratuito ("Spark"), permite: 1 GiB de almacenamiento, 50,000 lecturas/día y 20,000 escrituras/día. Cada documento (reporte, usuario) cuenta como una lectura cuando se consulta.

Para que un histórico grande de reportes no agote las lecturas diarias, el panel principal solo trae por defecto **los 2 reportes más recientes** (una consulta acotada, no toda la colección). Hay un botón **"Ver historial completo"** para traer todos los reportes cuando alguien lo necesite — esa consulta sí cuenta como una lectura por reporte guardado, pero solo ocurre cuando se pide explícitamente, y una vez traído queda en memoria durante esa sesión (no se vuelve a cobrar por alternar entre "recientes" y "completo" sin recargar la página).

Con esto, el uso diario normal (crear y revisar los últimos hallazgos) consume prácticamente nada de la cuota gratuita, sin importar cuántos reportes se acumulen con el tiempo.
